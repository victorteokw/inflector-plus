...
///
/// ```
pub fn to_word_case(non_word_case_string: &str) -> String {
    ...
    to_case_camel_like(non_word_case_string, options)
}
/// Determines of a `&str` is `Word case`
///
/// ```
///     use inflector::cases::wordcase::is_word_case;
///     let mock_string: &str = "foo-bar-string-that-is-really-really-long";
///     let asserted_bool: bool = is_word_case(mock_string);
...
///
/// ```
pub fn is_word_case(test_string: &str) -> bool {
    test_string == to_word_case(test_string.clone())
}
...
mod benchmarks {
    ...
    fn bench_word(b: &mut Bencher) {
        b.iter(|| super::to_word_case("Foo BAR"));
    }

    #[bench]
    fn bench_is_word(b: &mut Bencher) {
        b.iter(|| super::is_word_case("Foo bar"));
    }

    #[bench]
    fn bench_word_from_snake(b: &mut Bencher) {
        b.iter(|| super::to_word_case("foo_bar"));
    }
...
mod tests {
    ...
    fn from_camel_case() {
        ...
        assert_eq!(to_word_case(&convertable_string), expected)
    }
    ...
    fn is_correct_from_camel_case() {
        let convertable_string: String = "fooBar".to_owned();
        assert_eq!(is_word_case(&convertable_string), false)
    }
    ...
    fn is_correct_from_word_case() {
        let convertable_string: String = "foo bar".to_owned();
        assert_eq!(is_word_case(&convertable_string), true)
    }
